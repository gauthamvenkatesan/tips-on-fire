...
export const exercise = createSlice({
  name: 'exercise',
  initialState: {  progress: {courseId: 0,enteredChar: '',currentItemId : 0, mistakes : 0, status: "none"},
  ...
  reducers: {
    START_COURSE: (state, action) => {
      ...
      state.contentString  = action.course.contentString;
    },
    START_TYPING: (state, action) => {      
      ...
      state.progress = updatedProgressStartTyping;
      let contentObj = getContentObj(state.contentString, runnerLimit);
      state.content = contentObj.contentArr;
      state.contentConsumedIndex = contentObj.contentConsumedIndex;
      state.time = Number.parseInt(action.param.time);
      state.words = Number.parseInt(action.param.words)
    },
    KEY_PRESSED: (state, action) => {
      ...
      state.progress = progressUpdated           
    },
    SWITCH_BOUNDARY_MODE: (state, action) => {
      state.timeMode = action.mode === "time";
    },
    UPDATE_TIME_PASSED: (state, action) => {
      ...
      let currentItemId = state.progress.currentItemId;
      ...
      state.progress = progressUpdatedTimePassed;
    }
  },
})
...
export const {  KEY_PRESSED, START_COURSE, START_TYPING, UPDATE_TIME_PASSED, SWITCH_BOUNDARY_MODE } = exercise.actions
